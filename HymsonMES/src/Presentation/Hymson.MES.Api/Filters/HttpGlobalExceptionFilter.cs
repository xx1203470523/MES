using AutoMapper.Internal;
using FluentValidation;
using FluentValidation.Results;
using Hymson.Infrastructure.Exceptions;
using Hymson.Localization.Services;
using Hymson.Web.Framework.Constants;
using Microsoft.AspNetCore.Mvc;
using Microsoft.AspNetCore.Mvc.Filters;
using System.Collections.Generic;
using System.Net;
using System.Text;

namespace Hymson.WebApi.Filters
{
    /// <summary>
    /// 全局异常过滤器
    /// </summary>
    public class HttpGlobalExceptionFilter : IExceptionFilter
    {
        private readonly IWebHostEnvironment _env;
        private readonly ILogger<HttpGlobalExceptionFilter> _logger;
        private readonly ILocalizationService _localizationService;

        /// <summary>
        /// ctor
        /// </summary>
        /// <param name="env"></param>
        /// <param name="logger"></param>
        /// <param name="localizationService"></param>
        public HttpGlobalExceptionFilter(IWebHostEnvironment env,
            ILogger<HttpGlobalExceptionFilter> logger, ILocalizationService localizationService)
        {
            _env = env;
            _logger = logger;
            _localizationService = localizationService;
        }

        private static string TryToLocalizeExceptionMessage(Exception exception,string localizedValue) {
            if (exception.Data != null && exception.Data.Count > 0)
            {
                foreach (var key in exception.Data.Keys)
                {
                    localizedValue = localizedValue.Replace("{" + key + "}", exception.Data[key]?.ToString());
                }
            }
            return localizedValue;
        }

        private string TryToLocalizeValidationFailureMessage(ValidationFailure validationFailure)
        {
            string localizedValue = _localizationService.GetResource(validationFailure.ErrorCode);
            if (validationFailure.FormattedMessagePlaceholderValues != null && validationFailure.FormattedMessagePlaceholderValues.Any())
            {
                foreach (var keyValuePair in validationFailure.FormattedMessagePlaceholderValues)
                {
                    if (keyValuePair.Value == null) { continue; }
                    localizedValue = localizedValue.Replace("{" + keyValuePair.Key + "}", keyValuePair.Value.ToString());
                }
                try
                {
                    if (validationFailure.FormattedMessagePlaceholderValues.ContainsKey("PropertyValue"))
                    {
                        localizedValue = localizedValue.Replace("{" + validationFailure.PropertyName + "}", validationFailure.FormattedMessagePlaceholderValues["PropertyValue"].ToString());
                    }
                }
                catch (Exception ex)
                {
                    _logger.LogError(ex, "替换失败:");

                }
                
            }
            return localizedValue;
        }
        private void PrepareBadRequestObjectResult(BaseException exception, ExceptionContext context)
        {
            var problemDetails = new ValidationProblemDetails()
            {
                Instance = context.HttpContext.Request.Path,
                Status = StatusCodes.Status400BadRequest,
                Detail = "Please refer to the errors property for additional details."
            };
            problemDetails.Errors.Add(exception.ErrorCode,
                new string[] {
                    TryToLocalizeExceptionMessage(exception, _localizationService.GetResource(exception.ErrorCode)) });
            context.Result = new BadRequestObjectResult(problemDetails);
        }
        private void PrepareInternalServerErrorObjectResult(BaseException exception, ExceptionContext context) {
            var problemDetails = new InternalServerProblemDetails()
            {
                Instance = context.HttpContext.Request.Path,
                Status = StatusCodes.Status500InternalServerError,
            };
            if (_env.IsDevelopment())
            {
                problemDetails.Detail = context.Exception.Message;
            }
            problemDetails.Errors.Add(exception.ErrorCode,
                new string[] {
                    TryToLocalizeExceptionMessage(exception, _localizationService.GetResource(exception.ErrorCode)) });
            context.Result = new InternalServerErrorObjectResult(problemDetails);
        }
        /// <summary>
        /// 异常处理
        /// </summary>
        /// <param name="context"></param>
        public void OnException(ExceptionContext context)
        {
            _logger.LogError(new EventId(context.Exception.HResult),
            context.Exception,
            context.Exception.Message);

            if (context.Exception is ValidationException validationException)
            {
                var problemDetails = new ValidationProblemDetails()
                {
                    Instance = context.HttpContext.Request.Path,
                    Status = StatusCodes.Status400BadRequest,
                    Detail = "Please refer to the errors property for additional details."
                };
                if (validationException.Errors.Any(x => x.FormattedMessagePlaceholderValues.ContainsKey("CollectionIndex")))
                {
                    //分组合并
                    
                    Dictionary<object, List<ValidationFailure>> keyValuePairs = new();
                    foreach (var  validationFailure in validationException.Errors)
                    {
                        if (keyValuePairs.ContainsKey(validationFailure.FormattedMessagePlaceholderValues["CollectionIndex"]))
                        {
                            keyValuePairs[validationFailure.FormattedMessagePlaceholderValues["CollectionIndex"]].Add(validationFailure);
                        }
                        else
                        {
                            keyValuePairs[validationFailure.FormattedMessagePlaceholderValues["CollectionIndex"]] = new List<ValidationFailure>
                            {
                                validationFailure
                            };
                        }
                    }
                    StringBuilder errorMessages = new();
                    StringBuilder errorCodes = new();
                    //组装数据
                    foreach (var keyValuePair in keyValuePairs)
                    {
                        errorMessages.AppendFormat(validationException.Message, keyValuePair.Key);
                        foreach (var validationFailure in keyValuePair.Value)
                        {
                            errorCodes.AppendFormat(validationFailure.ErrorCode);
                            errorMessages.Append(TryToLocalizeValidationFailureMessage(validationFailure));
                            errorMessages.Append(',');
                        }
                        errorMessages.Length--;
                        problemDetails.Errors.Add($"{keyValuePair.Key}_{errorCodes}", new string[] { errorMessages.ToString() });
                        errorMessages.Clear();
                        errorCodes.Clear();
                    }

                }
                else
                {
                    foreach (var validationFailure in validationException.Errors)
                    {
                        problemDetails.Errors.Add(validationFailure.ErrorCode, new string[] { TryToLocalizeValidationFailureMessage(validationFailure) });
                    }
                }
                context.Result = new BadRequestObjectResult(problemDetails);
            }
            else if (context.Exception is CustomerValidationException customerValidationException)
            {
                PrepareBadRequestObjectResult(customerValidationException, context);
            }
            else if (context.Exception is NotFoundException notFoundException)
            {
                PrepareBadRequestObjectResult(notFoundException, context);

            }
            else if (context.Exception is BusinessException businessException)
            {
                PrepareInternalServerErrorObjectResult(businessException, context);
            }
            else if (context.Exception is CustomerDataException dataException)
            {
                PrepareInternalServerErrorObjectResult(dataException, context);
            }
            else if (context.Exception is ExcelDataValidationException excelDataValidationException) {
                var problemDetails = new ValidationProblemDetails()
                {
                    Instance = context.HttpContext.Request.Path,
                    Status = StatusCodes.Status400BadRequest,
                    Detail = "Please refer to the errors property for additional details."
                };
                StringBuilder errorMessages = new();
                StringBuilder errorCodes = new();
                foreach (var excelDataError in excelDataValidationException.ExcelDataErrors)
                {
                    var excelRowsFormat = _localizationService.GetResource("ExcelRows");
                    errorMessages.AppendFormat(excelRowsFormat, excelDataError.ExcelRows);
                    foreach (var validationFailure in excelDataError.ValidationFailures)
                    {
                        errorCodes.AppendFormat(validationFailure.ErrorCode);
                        errorMessages.Append(TryToLocalizeValidationFailureMessage(validationFailure));
                        errorMessages.Append(',');
                    }
                    errorMessages.Length--;
                    problemDetails.Errors.Add($"{excelDataError.ExcelRows}_{errorCodes}", new string[] { errorMessages.ToString()});
                    errorMessages.Clear();
                    errorCodes.Clear();
                }
                context.Result = new BadRequestObjectResult(problemDetails);
            }
            else
            {
                var problemDetails = new InternalServerProblemDetails()
                {
                    Instance = context.HttpContext.Request.Path,
                    Status = StatusCodes.Status500InternalServerError,
                };

                if (_env.IsDevelopment())
                {
                    problemDetails.Detail = context.Exception.Message;
                }
                problemDetails.Errors.Add(nameof(ErrorCode.SYS20000),
                    new string[] { _localizationService.GetResource(nameof(ErrorCode.SYS20000)) });
                context.Result = new InternalServerErrorObjectResult(problemDetails);
            }
            context.ExceptionHandled = true;
        }

        
    }
    public class InternalServerProblemDetails : ValidationProblemDetails { 
    
    }
    public class InternalServerErrorObjectResult : ObjectResult
    {
        public InternalServerErrorObjectResult(object error)
            : base(error)
        {
            StatusCode = StatusCodes.Status500InternalServerError;
        }
    }


}
