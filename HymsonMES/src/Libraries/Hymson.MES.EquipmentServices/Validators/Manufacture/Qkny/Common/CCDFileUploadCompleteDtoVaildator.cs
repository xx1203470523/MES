...
namespace Hymson.MES.EquipmentServices.Validators.Manufacture.Qkny.Common
{
    ...
    /// 验证器
    /// </summary>
    internal class CCDFileUploadCompleteDtoVaildator : AbstractValidator<CCDFileUploadCompleteDto>
    {
        /// <summary>
        /// 构造函数
        /// </summary>
        public CCDFileUploadCompleteDtoVaildator()
        {
            //设备编码不能为空
            ...
            //资源编码不能为空
            RuleFor(x => x.SfcList).NotNull().NotEmpty().Must(list => list.Any()).WithErrorCode(ErrorCode.MES45190);
        }
    }
}
