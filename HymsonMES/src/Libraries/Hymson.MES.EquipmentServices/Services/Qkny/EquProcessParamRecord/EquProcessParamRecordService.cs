...
namespace Hymson.MES.Services.Services.EquProcessParamRecord
{
    ...
    public class EquProcessParamRecordService : IEquProcessParamRecordService
    {
        ...
        /// <returns></returns>
        public async Task<int> CreateAsync(EquProcessParamRecordSaveDto saveDto)
        {
            // 判断是否有获取到站点码 
            ...
            // DTO转换实体
            var entity = saveDto.ToEntity<EquProcessParamRecordEntity>();
            ...
            // 保存
            return await _equProcessParamRecordRepository.InsertAsync(entity);
        }
        ...
        /// <param name="saveDto"></param>
        /// <returns></returns>
        public async Task<int> ModifyAsync(EquProcessParamRecordSaveDto saveDto)
        {
            // 判断是否有获取到站点码 
            if (_currentSite.SiteId == 0) throw new CustomerValidationException(nameof(ErrorCode.MES10101));
            ...
            return await _equProcessParamRecordRepository.UpdateAsync(entity);
        }
        ...
        {
            return await _equProcessParamRecordRepository.DeleteAsync(id);
        }
        ...
        {
            return await _equProcessParamRecordRepository.DeletesAsync(new DeleteCommand
            {
                Ids = ids,
        ...
        {
           var equProcessParamRecordEntity = await _equProcessParamRecordRepository.GetByIdAsync(id);
           if (equProcessParamRecordEntity == null) return null;
           
           return equProcessParamRecordEntity.ToModel<EquProcessParamRecordDto>();
        }
        ...
        /// <returns></returns>
        public async Task<PagedInfo<EquProcessParamRecordDto>> GetPagedListAsync(EquProcessParamRecordPagedQueryDto pagedQueryDto)
        {
            var pagedQuery = pagedQueryDto.ToQuery<EquProcessParamRecordPagedQuery>();
            pagedQuery.SiteId = _currentSite.SiteId ?? 0;
            var pagedInfo = await _equProcessParamRecordRepository.GetPagedListAsync(pagedQuery);

            // 实体到DTO转换 装载数据
            var dtos = pagedInfo.Data.Select(s => s.ToModel<EquProcessParamRecordDto>());
            return new PagedInfo<EquProcessParamRecordDto>(dtos, pagedInfo.PageIndex, pagedInfo.PageSize, pagedInfo.TotalCount);
        }
