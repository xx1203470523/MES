...
namespace Hymson.MES.CoreServices.Services.Job
{
    ...
    /// 供应商条码接收
    /// </summary>
    [Job("供应商条码接收", JobTypeEnum.Standard)]
    public class SupplierBarcodeReceiveService : IJobService
    {
        /// <summary>
        /// 日志对象
        /// </summary>
        ...
        public SupplierBarcodeReceiveService(ILogger<BarcodeReceiveService> logger,
        ...
        public async Task<object?> DataAssemblingAsync<T>(T param) where T : JobBaseBo
        {
            ...
            foreach (var sfc in multiSFCBo.SFCs)
            {
                ...
                if (whMaterialInventory == null)
                {
                    ...
                    if (!await _manuCommonService.CheckBarCodeByMaskCodeRuleAsync(sfc, productId))
                    {
                ...
                {
                    if (whMaterialInventory.QuantityResidue <= 0)
                    {
                        var validationFailure = new ValidationFailure();
                        if (validationFailure.FormattedMessagePlaceholderValues == null || !validationFailure.FormattedMessagePlaceholderValues.Any())
                        {
