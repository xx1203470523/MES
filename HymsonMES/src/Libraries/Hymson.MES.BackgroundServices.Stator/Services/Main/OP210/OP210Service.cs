...
namespace Hymson.MES.BackgroundServices.Stator.Services
{
    ...
    /// 服务
    /// </summary>
    public partial class OP210Service : IOP210Service
    {
        /// <summary>
        /// 日志接口
        /// </summary>
        private readonly ILogger<OP210Service> _logger;

        /// <summary>
        /// 仓储接口（工序）
        /// </summary>
        private readonly IOPRepository<OP210> _opRepository;
        ...
        public OP210Service(ILogger<OP210Service> logger,
            IOPRepository<OP210> opRepository,
        ...
        public async Task<int> ExecuteAsync(int limitCount)
        {
            ...
            _logger.LogInformation($"OP210 水位线；时间： {waterMarkId.ToString()}");
        ...
        private async Task<StatorSummaryBo> ConvertDataListAsync<T>(IEnumerable<T> entities, IEnumerable<string> barCodes, IEnumerable<string>? parameterCodes = null) where T : BaseOPEntity
        {
            ...
            foreach (var opEntity in entities)
            {
                ...
                summaryBo.ManuSfcCirculationEntities.Add(new ManuSfcCirculationEntity
                {
                    ...
                    CreatedBy = statorBo.User,
                    CreatedOn = statorBo.Time,
                    UpdatedBy = "OP210",//StatorConst.USER,
                    UpdatedOn = time
                });
