...
public partial class PlanWorkOrderRepository : BaseRepository, IPlanWorkOrderRepository
{
    ...
    /// <returns></returns>
    public async Task<PlanWorkOrderEntity> GetOneAsync(PlanWorkOrderQuery query)
    {
        ...
        return await conn.QueryFirstOrDefaultAsync<PlanWorkOrderEntity>(templateData.RawSql, templateData.Parameters);
    }
    ...
    /// <param name="query"></param>
    /// <returns></returns>
    public async Task<IEnumerable<PlanWorkOrderEntity>> GetListAsync(PlanWorkOrderQuery query)
    {
        var sqlBuilder = new SqlBuilder();
...
public partial class PlanWorkOrderRepository
{
    ...
    const string GetListSqlTemplate = "SELECT * FROM `plan_work_order` /**where**/;";
...
/// <returns></returns>
private static SqlBuilder WhereFill(SqlBuilder sqlBuilder, PlanWorkOrderQuery query)
{
    ...
    if (!string.IsNullOrWhiteSpace(query.OrderCodeLike))
    {
        query.OrderCodeLike = $"{query.OrderCodeLike}%";
        sqlBuilder.Where("OrderCode Like @OrderCodeLike");
    }
    ...
    if (query.WorkCenterType.HasValue)
    {
        sqlBuilder.Where("WorkCenterType = @WorkCenterType");
    }

    if (query.WorkCenterTypes != null && query.WorkCenterTypes.Any())
    {
        sqlBuilder.Where("WorkCenterType IN @WorkCenterTypes");
    }
    ...
    if (query.ProductBOMId.HasValue)
    {
        sqlBuilder.Where("ProductBOMId = @ProductBOMId");
    }

    if (query.ProductBOMIds != null && query.ProductBOMIds.Any())
    {
        sqlBuilder.Where("ProductBOMId IN @ProductBOMIds");
    }
    ...
    {
        sqlBuilder.Where("Type IN @Types");
    }
    ...
    if (query.OverScaleMin.HasValue)
    {
        sqlBuilder.Where("OverScale >= @OverScaleMin");
    }

    if (query.OverScaleMax.HasValue)
    {
        sqlBuilder.Where("OverScale <= @OverScaleMax");
    }


    if (query.IsLocked.HasValue)
    {
        sqlBuilder.Where("IsLocked = @IsLocked");
    }

    if (query.IsLockeds != null && query.IsLockeds.Any())
    {
        sqlBuilder.Where("IsLocked IN @IsLockeds");
    }
    ...
    if (query.LockedStatus.HasValue)
    {
        sqlBuilder.Where("LockedStatus = @LockedStatus");
    }

    if (query.LockedStatuss != null && query.LockedStatuss.Any())
    {
        sqlBuilder.Where("LockedStatus IN @LockedStatuss");
    }
