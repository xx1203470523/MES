...
#if DM
        const string MergeSql = @"MERGE INTO manu_sfc_summary AS targetTable USING((SELECT @Id) AS sourceTable(Id))  ON(targetTable.Id=sourceTable.Id ) WHEN MATCHED THEN UPDATE SET 
